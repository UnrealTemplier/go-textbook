...
Скорость света в вакууме составляет около $300\,000\text{ км/с}$, но в оптоволоконном кабеле из-за показателя преломления кварцевого стекла она падает примерно до $200\,000\text{ км/с}$. Пакет, летящий от пользователя из Сиднея к серверу во Франкфурте и обратно, преодолевает более $30\,000\text{ километров}$. Это означает неумолимый физический предел: минимальная сетевая задержка (RTT — Round Trip Time) составит не менее 150–200 миллисекунд, даже если ваш Go-бэкенд отвечает за рекордные 200 микросекунд.

Единственный способ победить скорость света — **не путешествовать на другой конец планеты**. Данные должны отдаваться с узла, расположенного в пяти километрах от пользователя.
...
flowchart LR
    ...
    subgraph GeoWorld ["Физическая география и задержка (RTT)"]
        User["Пользователь (Сидней)"]
        EdgePoP["Edge PoP (Сидней): RTT ~ 5 мс"]
        OriginSrv["Origin Go Сервер (Франкфурт): RTT ~ 200 мс"]
        
        User <-->|"5 мс (Cache HIT)"| EdgePoP
        EdgePoP -.->|"200 мс (Только при Cache MISS)"| OriginSrv
    end
```
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    Client["Клиент<br>(Браузер/Клиентский SDK):::entry"] -->|"DNS Resolve<br>(GeoDNS)"| Edge["Edge-узел<br>(Anycast IP)"]
...
- **ETag / If-None-Match**: Токен (обычно `md5` или `xxhash` от тела/метаданных). Если совпадает → `304 Not Modified` без передачи тела ответа, что экономит трафик и процессорное время.
- **Last-Modified / If-Modified-Since**: Таймстамп в UTC. Менее точный, но экономит CPU на вычислении хеша.

```mermaid
sequenceDiagram
    autonumber
    participant Client as Клиент / CDN
    participant Server as Go Origin Server

    Note over Client,Server: Первичный запрос (Cache Miss)
    Client->>Server: GET /api/catalog
    Server-->>Client: 200 OK (ETag: "v1-a4f8", Cache-Control: max-age=60, s-maxage=300)

    Note over Client,Server: Повторный запрос после истечения max-age (Условный запрос)
    Client->>Server: GET /api/catalog (If-None-Match: "v1-a4f8")
    Note over Server: Хэш совпадает! Тело ответа генерировать НЕ нужно!
    Server-->>Client: 304 Not Modified (ETag: "v1-a4f8", пустое тело)
```
...
- **Решение**: Используйте `sync.Once` или `singleflight` (`golang.org/x/sync/singleflight`) для дедупликации запросов к БД внутри одного процесса Go, а на уровне CDN — `stale-while-revalidate`.
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    subgraph SingleflightDedup ["Дедупликация через singleflight.Group в Go"]
        Req1["Горутина 1 (Запрос ключа A):::entry"] --> Group["singleflight.Group.Do('keyA')"]
        Req2["Горутина 2 (Запрос ключа A)"] --> Group
        Req3["Горутина 3 (Запрос ключа A)"] --> Group
        
        Group -->|"Ровно 1 запрос!"| DB["База данных / Тяжелый сервис"]
        DB -->|"Результат"| Group
        Group -->|"Рассылка одного результата всем горутинам"| Out["Ответ клиентам без нагрузки на БД"]
    end
```
...
func main() {
	...
	mux.HandleFunc("/api/data", cacheHandler(func(w http.ResponseWriter, r *http.Request) {}, 5*time.Minute, "v1-data"))
