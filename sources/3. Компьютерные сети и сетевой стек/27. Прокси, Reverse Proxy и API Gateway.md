...
flowchart LR
    subgraph FWD ["Прямой прокси (Forward Proxy)"]
        C1["Клиент A"] --> FP["Forward Proxy<br/>(Squid / Корпоративный)"]
        C2["Клиент B"] --> FP
        FP -->|"Один внешний IP"| Web["Публичный Интернет"]
    end

    subgraph REV ["Обратный прокси (Reverse Proxy)"]
        Internet["Клиенты из интернета"] --> RP["Reverse Proxy<br/>(Nginx / Envoy / Go)"]
        RP --> B1["Микросервис 1"]
        RP --> B2["Микросервис 2"]
        RP --> B3["Микросервис 3"]
    end
...
```mermaid
flowchart TD
    ClientReq["Входящий *http.Request"] --> DirectorHook["1. Director / Rewrite Hook:<br/>Модификация URL, Host, X-Forwarded-*"]
