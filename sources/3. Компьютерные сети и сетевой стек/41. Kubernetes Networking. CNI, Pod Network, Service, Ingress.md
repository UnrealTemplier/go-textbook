...
Облачные балансировщики и NAT-шлюзы (AWS NLB, GCP Cloud NAT, Azure LB) часто убивают idle TCP соединения через 350-1000 секунд без отправки RST-пакета. Go `http.Client` по умолчанию отключает keep-alive для `http://` (только `https` использует).
