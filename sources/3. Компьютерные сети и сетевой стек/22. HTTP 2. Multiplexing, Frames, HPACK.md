...
flowchart LR
    ...
    subgraph HTTP2 ["HTTP/2 (Бинарные структурированные фреймы)"]
        F1["9B Header | Stream 1 | HEADERS"]:::process
        F2["9B Header | Stream 3 | HEADERS"]:::process
        F3["9B Header | Stream 1 | DATA"]:::process
        F4["9B Header | Stream 3 | DATA"]:::process
    end
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    A["Фрейм HTTP/2"]:::network --> B["Длина 24 бита"]
    A --> C["Тип 8 бит"]
    A --> D["Флаги 8 бит"]
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    Client["Клиент"]:::entry -->|"TCP Connection"| Server["Сервер"]
...
flowchart TD
    ...
    subgraph HPACK ["Архитектура HPACK"]
        HIn["Входящий заголовок"]:::warning --> Check{"Есть в таблице?"}
