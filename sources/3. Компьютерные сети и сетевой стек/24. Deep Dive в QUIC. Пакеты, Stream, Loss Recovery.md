...
flowchart TD
    ...
    subgraph Userspace ["Пространство пользователя (Go Runtime)"]
        ...
        StreamEngine["Движок Streams & Flow Control"]:::process
        ...
        BufferPool["sync.Pool буферов памяти"]:::accent
...
flowchart LR
    ...
    subgraph PacketTypes ["Типы пакетов QUIC"]
        LHeader["Long Header (Рукопожатие):::entry"] --> LH1["Initial (Запуск TLS 1.3)"]
        LHeader --> LH2["0-RTT (Ранние данные)"]
        LHeader --> LH3["Handshake (Сертификаты)"]
        LHeader --> LH4["Retry (Защита от спуфинга)"]
        
        SHeader["Short Header (1-RTT Данные):::process"] --> SH1["Минимальный overhead"]
