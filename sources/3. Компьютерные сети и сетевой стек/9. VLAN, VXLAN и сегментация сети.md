...
flowchart TD
    ...
    subgraph Underlay ["Underlay-сеть (Физическая L3 IP-магистраль дата-центра)"]
        ...
        VTEP1 <-->|UDP/IP Туннель Port 4785| SpineSwitch
        SpineSwitch <-->|UDP/IP Туннель Port 4785| VTEP2
    end
...
flowchart LR
    ...
    OuterIP --> OuterUDP["Внешний UDP: Port 4785 (8B)"]
...
2. **Outer UDP Header (8 байт):** Порт назначения по стандарту IANA равен 4789, однако в подсистеме ядра Linux исторически по умолчанию используется порт **`4785`**. Исходный порт выбирается как хэш от внутренних заголовков пакета.
...
4. **Транспорт UDP (порт 4785):** Гарантирует бесшовный проход через NAT, поддержку аппаратного offload на сетевых картах и балансировку ECMP на физических фабриках ЦОД.
