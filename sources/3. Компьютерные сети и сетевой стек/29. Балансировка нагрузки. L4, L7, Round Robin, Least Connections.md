...
Для разработчика на PHP или классической Java балансировщик часто выглядит как «чёрный ящик», который просто раздает запросы между воркерами или серверами приложений. В Go модель конкурентности фундаментально меняет эту парадигму. Каждый HTTP-запрос в Go обрабатывается в изолированной **горутине** (гортине), которая ожидает I/O через `netpoll` (epoll/kqueue). 

Представьте себе операционный зал крупного банка с десятью окнами кассиров. Если администратор в зале направляет посетителей строго по очереди (первого — в окно №1, второго — в окно №2...), а кассир в первом окне внезапно сталкивается со сложным оформлением ипотеки на 45 минут, то следующие клиенты окна №1 окажутся в ловушке многочасового ожидания, в то время как соседние кассиры будут скучать. 
...
flowchart TD
    ...
    Client["Клиентский трафик (TCP/TLS):::network"] --> LB{"Тип балансировщика"}
    
    subgraph L4Path ["Layer 4 (Transport: IP + Port)"]
        LB -- "L4" --> L4Engine["Ядро Linux: conntrack / nftables / LVS / DPDK / XDP"]
        L4Engine -->|"Пакетная пересылка (SNAT/DNAT)"| B1["Go Pod 1"]
        L4Engine -->|"Без терминации TLS"| B2["Go Pod 2"]
    end
    
    subgraph L7Path ["Layer 7 (Application: HTTP / gRPC)"]
        LB -- "L7" --> L7Engine["Userspace Proxy (Nginx / Envoy / HAProxy)"]
        L7Engine -->|"TLS Termination + HTTP Routing"| B3["Go Pod 3"]
        L7Engine -->|"Headers / Cookies / Path"| B4["Go Pod 4"]
    end
```
...
* **Производительность:** Максимальная. Обработка одного пакета занимает микросекунды. Минимум контекстных переключений (context switches). Балансировщик выступает как регулировщик на скоростном шоссе, распределяя машины по полосам без досмотра содержимого багажника.
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    Client["Клиент (TCP/TLS):::entry"] -->|"SYN, ACK, Payload"| LB["Балансировщик"]
...
Добавляет коэффициент веса (capacity) каждой ноде. Позволяет учитывать разное железо в кластере (например, ноды с 8 ядрами получают в два раза больше запросов, чем ноды с 4 ядрами).
...
* **Механизм:** Использует виртуальное кольцо (virtual nodes). При добавлении/удалении ноды меняется только ~1/N часть трафика, а не 100% (как при простом хешировании `hash(key) % N`).
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    subgraph HashRing ["Кольцо Consistent Hashing с Virtual Nodes"]
        Ring["Виртуальное кольцо хэшей: [0 .. 2^32-1]"]
        N1A["Node A (vnode 1)"]
        N2A["Node B (vnode 1)"]
        N1B["Node A (vnode 2)"]
        N2B["Node B (vnode 2)"]
...
sequenceDiagram
    ...
    participant Srv as Go Service (Pod)

    Note over Srv: Получен сигнал SIGTERM от Kubernetes
    Srv->>LB: Health Check возвращает 503 Service Unavailable (или Draining)
    Note over LB: Исключение ноды из upstream-пула для новых запросов!
    
    Note over Srv: Вызов srv.Shutdown(ctx)
    Note over Srv: 1. Закрытие сокета Accept()<br/>2. Ожидание завершения активных горутин
    Srv-->>LB: Завершение ответов на in-flight запросы
    Note over Srv: Все горутины завершены, сокеты закрыты
    Note over Srv: Выход процесса с кодом 0
```
