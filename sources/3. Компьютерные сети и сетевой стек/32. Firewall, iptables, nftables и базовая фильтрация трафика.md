...
Когда Go-разработчик пишет привычную строчку кода:
```go
conn, err := net.DialContext(ctx, "tcp", "db.internal:5432")
```
легко поддаться иллюзии, будто между пользовательским процессом и удаленной базой данных протянут прямой, беспрепятственный провод. В реальности каждый сетевой байт проходит через бдительный пограничный контроль, встроенный в самое сердце ядра Linux — подсистему пакетной фильтрации.
...
Если сетевой экран настроен на действие `REJECT`, он вежливо и мгновенно возвращает клиенту TCP-сегмент с флагом `RST` (или ICMP Port Unreachable), и Go тут же получает ошибку `connection refused`. Но если фаервол настроен на действие `DROP`, он молча поглощает пакет, словно черная дыра. Клиентский код в Go зависает в ожидании ответа, пока не сработает локальный контекстный таймаут.
...
flowchart TD
    ...
    A["Пакет входит<br>в сетевой интерфейс"]:::process -->|"PREROUTING"| B{"Маршрутизация<br>операция"}
...
В кластерах Kubernetes с тысячами сервисов `kube-proxy` в режиме iptables создавал десятки тысяч правил, что приводило к заметному росту задержек (latency degradation) и загрузке ядер CPU исключительно на перебор цепочек.
...
Вместо жестко закодированных модулей расширений C `nftables` реализует компактную виртуальную машину в ядре Linux. Правила компилируются в байткод, исполняемый ядром на невероятной скорости.
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    subgraph OldIPTables ["iptables: O(N) Линейный перебор"]
        P1["Пакет"]:::process --> R1["Правило 1"] --> R2["Правило 2"] --> R3["..."] --> RN["Правило 5000"]
    end

    subgraph NewNFTables ["nftables: O(1) Хэш-сеты (Maps/Sets)"]
        P2["Пакет (IP):::process"] --> HSet{"Поиск в Hash Set ядра"}
        HSet -- "Найден за O(1)" --> Acc["ACCEPT"]
        HSet -- "Не найден" --> Drp["DROP"]
    end
```
...
*   **Динамические наборы**: Возможность автоматически добавлять IP в черный список при обнаружении аномалий.
...
Каждое активное соединение занимает память ядра (около 300–400 байт на запись) и хранится в двунаправленной хэш-таблице (tuple: `[SrcIP, DstIP, SrcPort, DstPort, Proto]`).
...
sequenceDiagram
    autonumber
    participant Client as Go Клиент
    participant Conntrack as Linux Kernel (conntrack)
    participant Server as Удаленный сервис (БД)

    Client->>Conntrack: TCP SYN (Инициализация сокета)
    Note over Conntrack: Проверка: nf_conntrack_count >= nf_conntrack_max!
    Note over Conntrack: Ошибка ядра: nf_conntrack: table full, dropping packet!
    Conntrack--xServer: Пакет сброшен! До сервера ничего не дошло!
    Note over Client: Go-приложение зависает в net.DialContext до истечения таймаута!
    Client->>Client: context.DeadlineExceeded (или ETIMEDOUT)
```
