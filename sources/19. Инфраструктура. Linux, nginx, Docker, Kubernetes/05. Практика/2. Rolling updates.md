...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    DeleteEvent["K8s решает удалить Pod"]:::process
    
    subgraph AppTrack["Ветка приложения (Локальная нода)"]
        Kubelet["Kubelet шлет SIGTERM Go-процессу"]:::process
        GoProc["Go-сервер получает сигнал и закрывает сокет"]:::network
        AppTrackEnd["💥 Сервер больше не слушает порт!"]:::network
        Kubelet --> GoProc --> AppTrackEnd
    end
    
    subgraph NetTrack["Сетевая ветка (Все ноды кластера)"]
        APIServer["API Server обновляет Endpoints"]:::network
        KubeProxy["Kube-proxy на 50 нодах замечает событие"]:::network
        IPTables["Пересчет и запись правил iptables / IPVS"]:::process
        NetTrackEnd["Трафик прекращает поступать на старый IP"]:::process
