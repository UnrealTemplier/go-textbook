...
Представьте старинную контору управляющего банком. Подписав важное распоряжение о переводе средств клиенту, банкир не срывается с места и не бежит лично на телеграф через весь город, бросая кабинет нараспашку. Если по дороге он попадет под проливной дождь или поскользнется на мостовой, банк останется без присмотра, а телеграмма так и не уйдет. Вместо этого банкир кладет подписанный документ в деревянный лоток с надписью «Исходящие» (Outbox) прямо на краю своего массивного дубового стола. Документ зафиксирован в амбарной книге и лотке единым движением руки. Через каждые пятнадцать минут в кабинет заходит штатный курьер, забирает накопившиеся депеши из лотка и относит их на телеграф. Даже если курьер задержится в пути, исходный документ надежно сохранен в стенах банка, а операция не потеряется.
...
flowchart TD
    ...
    Service["Go-сервис"]:::data --> DB["PostgreSQL<br>UPDATE order SET status='PAID'"]
...
- Реализовать duplicate detection, храня обработанные ID в своей БД (или используя идемпотентность самого брокера, если он поддерживает).
