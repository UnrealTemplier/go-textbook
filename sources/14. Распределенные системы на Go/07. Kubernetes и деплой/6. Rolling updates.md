...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    D["Deployment Controller\n(kube-controller-manager):::process"] -->|"Управляет старой версией"| RS1["ReplicaSet V1\nmyapp:v1"]
...
flowchart TD
    ...
    subgraph Calc["Математика Rolling Update (100 реплик, Surge 25%, Unavailable 25%)"]
        R["Желаемые реплики: 100"]:::process
        Min["Минимум доступных: 100 - 25 = 75 подов"]:::process
        Max["Максимум в кластере: 100 + 25 = 125 подов"]:::process
        R --> Min
        R --> Max
    end
```
