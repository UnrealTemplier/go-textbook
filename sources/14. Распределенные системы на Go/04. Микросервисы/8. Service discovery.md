...
flowchart TD
    ...
    subgraph ClientSide["Client-Side Discovery (Клиентская балансировка)"]
        C1["Go Client"]:::entry -->|"1. Lookup"| Reg1[("Service Registry")]
        Reg1 -->|"2. IP List"| C1
    ...
    subgraph ServerSide["Server-Side Discovery (Серверная балансировка)"]
        C2["Go Client"]:::entry -->|"1. Запрос на виртуальный адрес"| LB["Load Balancer / Kube-Proxy"]
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    Client["Go App"]:::network -->|"DNS Resolve: auth-srv"| DNS["CoreDNS"]
