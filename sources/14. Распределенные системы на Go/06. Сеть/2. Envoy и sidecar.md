...
flowchart LR
    ...
    subgraph Pod["Kubernetes Pod (Network Namespace: localhost)"]
        direction TB
        App["Go Application<br/>(user-service):::entry"] <-->|"syscall.Read/Write<br/>(Чистый HTTP)"| Envoy["Envoy Sidecar<br/>(Worker Threads: epoll)"]
    end
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    GoApp["Go-приложение (user-service):::entry"] -->|"1. syscall.Write в сокет :80"| Kernel["Ядро Linux (Network Namespace)"]
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    LDS["LDS<br/>(Listener Discovery):::process<br/>Слушающий порт :15001"] --> RDS["RDS<br/>(Route Discovery)<br/>Сопоставление хостов и путей"]
