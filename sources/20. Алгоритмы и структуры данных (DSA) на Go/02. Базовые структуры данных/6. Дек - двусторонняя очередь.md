## Дек: симметрия двусторонней очереди и алгоритм Work Stealing

В предыдущих статьях мы детально препарировали две фундаментальные абстракции: [[4. Стек]] с его строгим принципом LIFO («последним пришел — первым ушел») и [[5. Очередь]] с ее справедливой моделью FIFO («первым пришел — первым ушел»). Но архитектура реальных высокопроизводительных сервисов, рантаймов и планировщиков задач часто требует универсальных гибридных подходов.

Что делать, если системе требуется обрабатывать входящие события в порядке очереди (FIFO), но в случае сбоя или временной недоступности внешней базы данных необходимо вернуть задачу обратно в самое начало очереди без потери приоритета? Или как реализовать распределенный планировщик вычислений, где свободные процессорные ядра могут безопасно «красть» задачи у перегруженных соседей?

Для таких задач применяется **Дек (Deque — Double-Ended Queue, двусторонняя очередь)**.

---

## Архитектура Дека

Дек снимает односторонние ограничения стека и очереди, предоставляя симметричную возможность добавлять и извлекать элементы с **обоих концов** за константное время $O(1)$.

Контракт двусторонней очереди базируется на четырех ключевых операциях:
1.  `PushFront` — добавить элемент в начало (Голову / Head).
2.  `PushBack` — добавить элемент в конец (Хвост / Tail).
...
flowchart LR
    ...
    PushFront["PushFront<br/>Вставка в голову"]:::process --> Head["Head (Голова)"]
    Head --> PopFront["PopFront<br/>Извлечение из головы"]
    
    Head --- E1["Элемент 1"] --- E2["Элемент 2"] --- E3["Элемент 3"] --- Tail["Tail (Хвост)"]
    
    PushBack["PushBack<br/>Вставка в хвост"] --> Tail
    Tail --> PopBack["PopBack<br/>Извлечение из хвоста"]
```
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    subgraph RingBufferState["Динамический кольцевой буфер в срезе"]
        direction LR
        b0["buf[0] = C (tail)"] --- b1["buf[1] = free"] --- b2["buf[2] = free"] --- b3["buf[3] = A (head)"] --- b4["buf[4] = B"]
    end
    HeadPtr["Head = 3"] --> b3
    TailPtr["Tail = 0"] --> b0
```
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    subgraph RingGrow["Развертывание разорванного кольца при grow()"]
        direction TB
        OldRing["Старый буфер (tail <= head): [0..tail-1] и [head..len-1]"]
        NewLinear["Новый удвоенный буфер:<br/>[head..len-1] скопировано в начало<br/>[0..tail-1] пристыковано следом<br/>head сброшен в 0, tail = size"]
    end
    OldRing -->|"copy() в два шага"| NewLinear
```
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    subgraph DequeLayout["Дек загруженного потока (P)"]
        Head["Голова<br/>(Старые задачи)"] --- Middle["..."] --- Tail["Хвост<br/>(Свежие задачи)"]
    end
    
    Owner["Владелец потока (P/M)"]
    Thief["Свободный поток (Вор)"]
    
    %% Владелец работает с хвостом в обе стороны (как Стек LIFO)
    Owner -->|"PushBack (кладет задачу)"| Tail
    Tail -->|"PopBack (берет задачу)"| Owner
    
    %% Вор работает с головой только на извлечение (как Очередь FIFO)
    Head -->|"PopFront (крадет задачу)"| Thief
```
