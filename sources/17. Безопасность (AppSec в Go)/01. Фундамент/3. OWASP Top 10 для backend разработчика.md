...
Проект **OWASP Top 10** (Open Web Application Security Project) в глазах многих начинающих разработчиков выглядит как унылый бюрократический чек-лист, составленный аудиторами ради заполнения отчетов о соответствии. Однако опытный инженер видит в нем нечто совсем иное: это квинтэссенция боли, инцидентов и реальных взломов сотен тысяч продакшен-систем по всему миру.

Версия OWASP Top 10 2021 года сместила фокус с чисто технических уязвимостей на **архитектурные и процессные ошибки (Insecure Design) и сбои в конфигурации окружения (Security Misconfiguration)**.

Для бэкенд-разработчика на Go это означает, что защита от большинства пунктов закладывается на уровне дизайна системы, работы с рантаймом и конфигурации сетевых примитивов. Go — язык со статической типизацией и автоматическим управлением памятью, который надежно защищает нас от классического переполнения стекового буфера в стиле C/C++. Но это порождает опасную иллюзию неуязвимости. Компилятор Go бессилен, если программист забыл проверить права владения ресурсом в хэндлере, открыл системный профайлер `pprof` на внешнем порту, допустил атаку по времени при валидации токена или позволил неконтролируемому вводу исчерпать память кучи и уронить процесс в OOM.
...
flowchart TD
    ...
    style B fill:#7f1d1d,stroke:#f87171,color:#f8fafc
    style D fill:#7f1d1d,stroke:#f87171,color:#f8fafc
    style K fill:#7f1d1d,stroke:#f87171,color:#f8fafc
```

В этой статье мы подробно препарируем каждую из десяти категорий через призму реализации на Go, обращая внимание на поведение рантайма, работу с памятью, системные вызовы и типичные ловушки, которые всплывают на хардовых собеседованиях.

---

### 1. Broken Access Control

Класс уязвимостей, когда приложение не проверяет права доступа к ресурсам. В Go это часто проявляется из-за небрежной работы с `context` или отсутствия централизованного слоя авторизации.
...
Go статически линкует зависимости, поэтому уязвимость в библиотеке становится частью вашего бинарника. В отличие от PHP или Java, где зависимости загружаются динамически или изолируются контейнером, в Go вы обязаны проверять `go.mod`, `go.sum` и прокси-кэш.
...
flowchart TD
    ...
    style J fill:#7f1d1d,stroke:#f87171,color:#f8fafc
```
...
2. **Медленные запросы (Slowloris)** → горутины (`g`) блокируются в `syscall read` (вызов `syscall.Read`). Планировщик `runtime` видит, что `P` простаивает, и создаёт новые `M` (треды ОС). Лимит тредов `GOMAXPROCS` не спасает, так как треды создаются динамически. Результат: исчерпание `ulimit -n` (файловые дескрипторы) или `vm.max_map_count`.
...
В следующей лекции мы углубимся в один из самых фундаментальных дуализмов безопасности: [[4. Аутентификация vs авторизация]].