...
> *«Простота — непременное условие надежности. Сложные системы всегда ломаются сложным образом; простые системы, как правило, работают».*    
> — Эдсгер Дейкстра
...
flowchart TD
    ...
    subgraph ClientsPub["Publishers (Отправители)"]
        P1["Order Service"]:::process
        P2["User Service"]:::entry
    end
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    Msg["Сообщение в: news.sports.football"]:::process
    
    W1["Подписка: news.sports.*"]:::process -->|"Матч! (ровно 1 токен)"| R1["Получено!"]
    W2["Подписка: news.>"]:::process -->|"Матч! (все уровни)"| R2["Получено!"]
    W3["Подписка: news.*"]:::process -->|"Мимо! (пропущен уровень)"| R3["Не получено!"]
