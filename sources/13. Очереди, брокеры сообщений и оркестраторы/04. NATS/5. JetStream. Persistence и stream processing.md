...
> *«Самый быстрый ввод-вывод — тот, которого вовсе не происходит. Когда же сохранение на диск неизбежно, пишите последовательно, агрессивно пакетируйте данные и держите протокол координации столь же простым, как сама физика диска».*    
> — Брайан Керниган
...
graph TB
    ...
    style JS fill:#1e293b,stroke:#6366f1,stroke-width:2px,color:#f8fafc
    style ST fill:#131d33,stroke:#38bdf8,stroke-width:2px,color:#f8fafc
    style CT fill:#131d33,stroke:#10b981,stroke-width:2px,color:#f8fafc
    style DS fill:#0f172a,stroke:#f59e0b,stroke-width:2px,color:#f8fafc
```
...
flowchart TD
    ...
    subgraph PushModel["Push Consumer (Реактивный поток)"]
        S1["JetStream Log"]:::network -->|"Проталкивание по TCP<br>(Flow Control & Heartbeats)"| C1["Worker: OnMessage()"]
    end
    
    subgraph PullModel["Pull Consumer (Контроль нагрузки / Backpressure)"]
        C2["Worker: Fetch(Batch=10):::entry"] -->|"Явный запрос"| S2["JetStream Log"]
