...
> *«Главное достоинство хорошего транспортного уровня в том, что он не путается под ногами и дает байтам летать. Но когда вам требуется память, приходится платить по счетам физики».*    
> — Брайан Керниган
...
flowchart LR
    ...
    P["Publisher (Отправитель):::network"] -->|"Send to 'orders'"| S["NATS Server<br>(In-Memory Only)"]
...
flowchart TD
    ...
    subgraph JetStreamServer["NATS Server (JetStream Mode)"]
        JS["JetStream Engine"]:::process
        Raft["Raft Consensus"]:::process
