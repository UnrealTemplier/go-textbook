...
> *«Имя переменной, функции или канала должно просто и ясно говорить о том, чем они являются. Грамотные пространства имен делают сложные архитектуры самодокументируемыми».*    
> — Брайан Керниган
...
flowchart TD
    ...
    subgraph Publisher["Событие от продюсера: news.sports.football.nfl"]
        direction TB
        Msg["Subject: news.sports.football.nfl"]:::process
    end

    subgraph MatchCheck["Проверка совпадения шаблонов (Pattern Matching)"]
        direction TB
        M1["Шаблон: news.sports.*.*"]:::process
        M2["Шаблон: news.sports.>"]:::process
        M3["Шаблон: news.sports.*"]:::process
        M4["Шаблон: news.>"]:::process
    end
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    P["Publisher: orders.new"]:::network --> S["NATS Server"]
