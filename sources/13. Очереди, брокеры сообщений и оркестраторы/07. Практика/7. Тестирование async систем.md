...
flowchart LR
    ...
    subgraph "Синхронный мир (Детерминизм)"
        T1["Test Worker"]:::network -->|"HTTP POST /order"| H1["Handler"]
        H1 -->|"DB Write"| DB1[("PostgreSQL")]
        H1 -->|"200 OK"| T1
    end

    subgraph "Асинхронный мир (Временной разрыв)"
        T2["Test Producer"]:::data -->|"Publish(msg)"| B[("Broker Queue")]
...
flowchart TD
    ...
    subgraph "Пирамида тестирования очередей"
        L1["Уровень 1: Unit & Fuzzing<br>• Чистые обработчики бизнес-логики<br>• Мокирование репозиториев и шины<br>• Скорость: микросекунды | 100% покрытие веток"]:::process
