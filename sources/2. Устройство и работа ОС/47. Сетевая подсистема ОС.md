...
flowchart LR
    ...
    subgraph Hardware["Физический уровень"]
        NIC["Сетевая карта (NIC):::process
10G / 40G / 100G"]
    end
    subgraph Kernel["Ядро Linux (Network Stack)"]
        Ring["RX Ring Buffer (DMA)"]
        NAPI["NAPI & SoftIRQ (net_rx_action)"]
        SKB["Аллокация struct sk_buff"]:::process
        L3L4["Маршрутизация IP & TCP стек"]:::network
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    A["NIC Hardware"]:::process -->|"Ethernet Frame"| B["NIC Driver"]
...
flowchart TD
    ...
    subgraph SockStructure["Анатомия struct sock"]
        State["Состояние: TCP_ESTABLISHED / TCP_TIME_WAIT"]:::process
...
(резерв на struct sk_buff)"]
        Cap["Ядро: обрезка по net.core.rmem_max"]:::system
        Actual["Фактический sk_rcvbuf в struct sock"]:::process
