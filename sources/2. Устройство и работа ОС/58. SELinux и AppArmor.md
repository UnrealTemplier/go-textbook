...
flowchart LR
    ...
    subgraph Syscalls ["Системные вызовы"]
        VFS_Op["Файлы: openat, read, write"]:::process
        Net_Op["Сеть: socket, bind, connect"]:::network
        Task_Op["Процессы: execve, fork, ptrace"]:::process
        IPCOp["IPC: shmget, msgsnd"]:::process
    end

    subgraph KernelCore ["Подсистемы ядра Linux"]
        VFS["VFS Subsystem"]:::process
        NetStack["Сетевой стек"]:::process
        SchedCore["Планировщик (sched)"]
        IPCCore["Подсистема IPC (ipc)"]
    end

    subgraph LSMFramework ["Каркас LSM (security_hook_heads)"]
        HookChain["Цепочка зарегистрированных хуков"]:::process
        SELinuxMod["Модуль SELinux"]:::process
        AppArmorMod["Модуль AppArmor"]:::process
        BPFMod["Модуль BPF-LSM"]:::process
    end
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    AppBinary["/usr/bin/myapp (Go Executable):::system"] --> KernelLoader["Ядро: execve()"]
