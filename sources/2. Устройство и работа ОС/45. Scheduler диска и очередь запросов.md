...
flowchart TD
    ...
    App1["Go Process (WAL fsync):::data"] -->|"syscall write/fsync"| VFS["VFS & Page Cache"]
...
flowchart TD
    ...
    subgraph MQDeadline["Алгоритм mq-deadline"]
        ...
        Dispatch{"Истек дедлайн запроса?"}:::warning
...
flowchart TD
    ...
    subgraph SoftwareQueues["Программные очереди (Software Staging Queues - per CPU)"]
        SQ0["CPU 0: Software Queue"]:::data
        SQ1["CPU 1: Software Queue"]:::data
        SQ2["CPU 2: Software Queue"]:::data
        SQN["CPU N: Software Queue"]:::data
    end

    subgraph HardwareQueues["Аппаратные очереди (Hardware Submission Queues - per NVMe Controller)"]
        HQ0["Hardware Queue 0"]:::data
        HQ1["Hardware Queue 1"]:::data
        HQM["Hardware Queue M"]:::data
    end
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    App["Go app: WriteAt():::data"] -->|"1. Буферизация"| PC["Page Cache (struct page)"]
