...
> *«Крепость с неприступными стенами бесполезна, если ее ворота слишком узки, чтобы пропустить собственную армию».*    
...
flowchart TD
    ...
    Client["Входящие клиенты (SYN):::warning"] --> PreFilter{"Проверка SYN Queue"}
...
flowchart TD
    ...
    subgraph ListenSocket["Слушающий сокет (struct sock)"]
        Flags["Флаги: SOCK_LISTEN"]:::process
        ListenSock["struct listen_sock (backlog, qlen, max_qlen)"]
        ReqQueue["struct request_sock_queue"]:::process
    end
