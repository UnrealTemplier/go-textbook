Программа, в которой параметры жестко зашиты в исходный код, лишена гибкости. Программа, конфигурация которой разбросана по глобальным переменным, магическим аннотациям или нетипизированным хэш-таблицам, смертельно опасна для продакшена.

В классической традиции Unix, заложенной в Bell Labs, механизм всегда отделяется от политики. Серверный бинарник должен содержать надежный механизм обработки сетевых запросов и бизнес-логики, но конкретная политика его поведения — сетевые порты, адреса баз данных, размер пулов соединений и таймауты — должна определяться конфигурацией во время запуска.

В этой лекции мы разберем, как построить надежную, строго типизированную и безопасную систему конфигурации на Go, сочувствующую памяти и процессору.
...
graph LR
    ...
    classDef cli fill:#0f172a,stroke:#6366f1,stroke-width:2px,color:#f8fafc;
    classDef merge fill:#1e293b,stroke:#38bdf8,stroke-width:2px,color:#f8fafc;
    classDef dest fill:#064e3b,stroke:#10b981,stroke-width:2px,color:#f8fafc;
    class A,C,D cli;
    class B merge;
    class E,F dest;
```
