...
flowchart TD
  ...
  A["Обработка элемента потока"]:::warning --> B{"Элемент уже в мониторе?"}
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
  A["Горутина 1 пишет в table[10]:::data"] --> B["Инвалидация кэш-линии 64 байта"]
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
  A["Типичные ошибки Heavy Hitters"]:::error --> B["Использование float64 для вероятностей"]
