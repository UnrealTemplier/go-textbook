...
flowchart TD
  ...
  A["Получение ленты: getNewsFeed(userId):::entry"] --> B["Сбор списка следящих<br>followeeIds + сам user"]
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
  A["Обычный пользователь"]:::entry --> B["Fan-out on Write<br>Push в кэш подписчиков"]
  A --> C["Чтение: O-1 из кэша"]
  D["Звезда с 10M фанатов"]:::process --> E["Fan-out невозможен<br>Write amplification"]
