...
flowchart TD
    ...
    A["Начало парсинга строки"]:::warning --> B{"Символ является цифрой?"}
...
flowchart LR
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    A["Итерация + явный стек"]:::error --> B["Память в куче<br>Контролируемый рост<br>Нет stack overflow<br>Кэш-локальность стека"]
    C["Рекурсия"]:::error --> D["Память на стеке горутины<br>Автоматический рост стека<br>Риск panic при глубине > 10K<br>Сложнее профилировать"]
```
