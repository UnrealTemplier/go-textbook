...
flowchart TD
    ...
    subgraph ClientRequest["Входящий запрос"]
        Req["GET /users/42"]:::process
    end

    subgraph Router["http.ServeMux (Radix Tree Matcher)"]
        Tree["Корень префиксного дерева"]:::process
        UsersNode["Узел: /users/"]:::process
    ...
    subgraph Target["Маршрутизация"]
        Match["Метод: GET -> Вызов UsersAPI.getUser"]:::process
        MismatchMethod["Метод: POST -> 405 Method Not Allowed"]:::process
        MismatchPath["Путь: /posts -> 404 Not Found"]:::error
    end
