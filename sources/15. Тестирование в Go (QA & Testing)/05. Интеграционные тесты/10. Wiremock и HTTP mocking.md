...
flowchart TD
    ...
    subgraph WireMockEngine["WireMock Server (Docker Container)"]
        Router["HTTP Listener & Request Router"]
        
        subgraph MatchingPipeline["Движок сопоставления (Radix Tree + JSONPath)"]
            M1["Matcher 1: Path /v1/payments<br>Header: Authorization: Bearer X<br>JSONPath: $.amount == 100"]
            M2["Matcher 2: Path /v1/payments<br>Fault: ConnectionReset"]
            MDefault["Fallthrough: Запрос не распознан"]
        end
        ...
        Resp1["HTTP 201 Created<br>Status: approved"]
        Resp2["TCP RST (Connection Reset)"]
        Resp404["HTTP 404 Not Found<br>WireMock Stub Mismatch Report"]
