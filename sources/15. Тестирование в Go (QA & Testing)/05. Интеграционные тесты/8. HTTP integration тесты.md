...
flowchart TD
    ...
    subgraph Mode1["Режим 1: In-Memory (httptest.NewRecorder)"]
        Test1["Тест go test"]:::data -->|"Прямой вызов ServeHTTP"| Rec["ResponseRecorder (bytes.Buffer)"]
        Rec <--> Hand1["HTTP Handler / Router"]
    end

    subgraph Mode2["Режим 2: Настоящая сеть (httptest.NewServer)"]
        Test2["Тест go test"]:::entry -->|"http.Client (sys_connect)"| Sock["Сетевой стек ОС (Loopback TCP)"]
