...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    E["Стратегии ошибок"]:::process --> S1["1. Sentinel Errors<br>(Глобальные маркеры: io.EOF, sql.ErrNoRows)"]
...
flowchart TD
    ...
    classDef network  fill:#881337,stroke:#fb7185,stroke-width:1px,color:#f8fafc
    classDef accent   fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#f8fafc
    A["Вызывающий код<br>errors.As(err, &dbErr):::error"] -->|"Unwrap()"| B["Пакет репозитория<br>fmt.Errorf('GetUser: %w', err)"]
